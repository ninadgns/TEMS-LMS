...
const EditExamInfo = ({ params, onRefresh }: { params: { initialData: ExamInfo }, onRefresh: () => void }) => {
  ...
  useEffect(() => {
    if (params.initialData) {
      form.reset(params.initialData); // Reset form with initial data
    }
  }, [params.initialData]);
  ...
  const deleteExam = async (event: React.MouseEvent<HTMLButtonElement>) => {
    event.preventDefault()
    if (!params.initialData.id) return
    const { data, error } = await supabase
      .from("Exams")
      .delete()
      .eq("id", params.initialData.id)
    if (error)
      toast({
        title: "Error",
        description: (
          <pre className="mt-2 w-[340px] rounded-md bg-slate-950 p-4">
            <code className="text-white">{error.details}</code>
          </pre>
        ),
      })
    else {
      toast({
        title: "Exam Deleted Succesfully "
      })
      router.push('/exams')
    }
            ...
            <DialogDescription>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2 m-5">
                  ...
                  <Button className="mx-2" onClick={deleteExam} variant={"destructive"}>Delete Exam</Button>
                </form>
              </Form>
            </DialogDescription>
          </DialogHeader>
